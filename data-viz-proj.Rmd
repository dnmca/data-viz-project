...
ggplot(data = grouped_by_date_patch, aes(x = date, y = mmmc, color = inside_GPGP)) +
  ...
  geom_text(data = data.frame(x = as.Date('1975-01-01'), y = 1750, label = "Average month microplastic\n    mass concentration"), 
            aes(x = x, y = y, label = label), size = 8, family = "Ubuntu Mono", color = "black", hjust = 0) +
  geom_text(data = data.frame(x = as.Date('1972-12-01'), y = 1375, label = "(among all available observation points in Pacific ocean)"), 
            aes(x = x, y = y, label = label), size = 4, family = "Ubuntu Mono", color = "grey30", hjust = 0) +
  theme(
    ...
    legend.text = element_text(family = 'Ubuntu Mono', size = 15),
...
ggsave('final/mmmc.png', last_plot(), width = 10)
```
...
ggsave('final/mmnc.png', last_plot(), width = 10)
```
...
ggplot() + 
  ...
  coord_fixed() +
  theme() +
  geom_text(data = data.frame(x = -12, y = 85, label = "Microplastic numerical concentration in Pacific ocean"), 
            aes(x = x, y = y, label = label), size = 6, family = "Ubuntu Mono", color = "black", hjust = 0) +
...
#ggsave("final/pacific_garbage_patch.png", last_plot(), width = 10, height = 10)
```
...
ggplot(data = countries, aes(x = long.new, y = lat)) +
  ...
  geom_text(data = data.frame(x = -20, y = 95, label = "Mismatched plastic waste per coutry per year, tonnes"), 
            aes(x = x, y = y, label = label), size = 5, family = "Ubuntu Mono", color = "black") +
  geom_point(data = SAG, 
  ...
  geom_text(x=35, y=12, size=3, label="GREAT PACIFIC \nGARBAGE PATCH", family = "Ubuntu Mono", color="grey40") +
  geom_text(x=-174, y=-50, size=3, label="SOUTH ATLANTIC \nGARBAGE PATCH", family = "Ubuntu Mono", color="grey40") +
  geom_text(x=-80, y=-49, size=3, label="INDIAN OCEAN \nGARBAGE PATCH", family = "Ubuntu Mono", color="grey40") +
  geom_text(x=70, y=-49, size=3, label="SOUTH PACIFIC\nGARBAGE PATCH", family = "Ubuntu Mono", color="grey40") +
  geom_text(x=152, y=13, size=3, label="NORTH \nATLANTIC\nGARBAGE\nPATCH", family = "Ubuntu Mono", color="grey40")
...
ggsave("final/mismatched.png", last_plot( ), width = 14)
```
...
```{r}
names(grouped_by_status) = c('economic_status', " Average plastic waste generation per day, kg", " Average amount of inadequately manged plastic waste per day, kg")
```
...
grouped_by_status %>% 
  ...
  theme(
    ...
    legend.text = element_text("Ubuntu mono"), 
    ...
    axis.title.x.bottom = element_text(family = 'Ubuntu Mono', vjust = -5, size = 13),
    axis.title.y.left = element_text(family = 'Ubuntu Mono', vjust = 5, size = 13),
    plot.margin = unit(c(3,3,3,3), "cm")
  ) + 
  ylab("kg/day") +
  xlab("Economic status")

ggsave('final/histogram.png', last_plot(), width = 10)
```
